...
const phish = function (names) {
  let bandRoster = names;
  let index = -1;
  const listNames = function () {
    index++;
    return bandRoster[index] + ' is a member of Phish';
  };
  return listNames;
};

let group = ['Trey', 'Page', 'Mike', 'Jon'];

let listRoster = phish(group);

for (let index = 0; index < group.length; index++) {
  let messageToBeLogged = listRoster();
