...
const fakePerson = function(number){
  let pessNumber = number;
  function Person (){
  ...
  const create = function (number) {
    let newPerson = [];
    for (let i = 0; i < number; i++) {
      newPerson.push(new Person());
    }
    return newPerson;
  };
return create(pessNumber);
};

const creatPeople = function (number){
  let result  = fakePerson(number);
  console.log(result);
};
